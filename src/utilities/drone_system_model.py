...
def create_system_model(
    ...
    drone_dynamics_pre_bound = partial(drone_dynamics,
                                       env=env,
                                       env_idx=env_idx)
    ...
    system_model = NonlinearSystem(f=drone_dynamics_pre_bound,
                                   h=drone_output,
                                   n=n,
                                   m=m,
                                   p=p,
...
def drone_tracking_callback(
    ...
    if current_distance - initial_distance > distance_threshold:
        raise ValueError("Drone Drone moved away from its goal by "
                         f"{distance_threshold} from its starting position.")
